...
function scoreRecommendations(quizData: QuizData): Recommendation[] {
  ...
  // Fonction d'évaluation des symptômes avec utilisation des mappings
  if (quizData.symptoms && quizData.symptoms.length > 0) {
    scoredRecommendations.forEach(rec => {
      ...
      quizData.symptoms?.forEach(symptom => {
        ...
        if (relatedRecommendations.includes(rec.id)) {
          rec.relevanceScore += 0.4; // Higher score for direct mapped relationships
        }
        
        // Fallback to category matching
        if (symptom === 'fatigue' && (rec.categories.includes('énergie') || rec.id === 'vitamin-b-complex')) {
          rec.relevanceScore += 0.3;
        }
        if (symptom === 'digestive problems' && rec.categories.includes('digestion')) {
          rec.relevanceScore += 0.4;
        }
        if (symptom === 'sleep_issues' && (rec.categories.includes('sommeil') || rec.id === 'magnesium-glycinate')) {
          rec.relevanceScore += 0.3;
        }
        if (symptom === 'stress' && (rec.categories.includes('stress') || rec.id === 'adaptogenic-herbs' || rec.id === 'mindfulness-meditation')) {
          rec.relevanceScore += 0.35;
        }
        if (symptom === 'anxiety' && (rec.categories.includes('stress') || rec.id === 'adaptogenic-herbs' || rec.id === 'mindfulness-meditation')) {
          rec.relevanceScore += 0.35;
        }
        if (symptom === 'weak_immune_system' && rec.categories.includes('immunité')) {
          rec.relevanceScore += 0.4;
        }
        if (symptom === 'skin_problems' && rec.categories.includes('peau')) {
          rec.relevanceScore += 0.3;
        }
        if (symptom === 'inflammation' && (rec.categories.includes('inflammation') || rec.id === 'omega3-supplementation')) {
          rec.relevanceScore += 0.4;
        }
        if (symptom === 'immunite' && rec.categories.includes('immunité')) {
          rec.relevanceScore += 0.4;
        }
        if (symptom === 'concentration' && (rec.categories.includes('fonction cognitive') || rec.id === 'omega3-supplementation')) {
          rec.relevanceScore += 0.3;
        }
        // Support for sleep issues under different terms
        if ((symptom === 'sleep issues' || symptom === 'Troubles du sommeil') && 
            (rec.categories.includes('sommeil') || rec.id === 'magnesium-glycinate' || rec.id === 'circadian-rhythm-optimization')) {
  ...
  // Évaluation des objectifs avec utilisation des mappings
  if (quizData.objectives && quizData.objectives.length > 0) {
    scoredRecommendations.forEach(rec => {
      // Use mapping data for objective-based recommendations
      quizData.objectives?.forEach(objective => {
        const relatedRecommendations = GOAL_RECOMMENDATIONS[objective] || [];
        if (relatedRecommendations.includes(rec.id)) {
          rec.relevanceScore += 0.45; // Strong boost for goal-aligned recommendations
        }
        
        // Fallback to direct objective matching
        if (objective === 'immune_boost' && rec.categories.includes('immunité')) {
          rec.relevanceScore += 0.4;
        }
        if (objective === 'digestion' && rec.categories.includes('digestion')) {
          rec.relevanceScore += 0.4;
        }
        if (objective === 'energy' && (rec.id === 'vitamin-b-complex' || rec.id === 'circadian-rhythm-optimization')) {
          rec.relevanceScore += 0.35;
        }
        if (objective === 'stress_management' && (rec.id === 'adaptogenic-herbs' || rec.id === 'mindfulness-meditation')) {
          rec.relevanceScore += 0.5;
        }
        if (objective === 'sleep_improvement' && (rec.id === 'magnesium-glycinate' || rec.id === 'circadian-rhythm-optimization')) {
          rec.relevanceScore += 0.45;
        }
        if (objective === 'mental_clarity' && (rec.id === 'omega3-supplementation' || rec.id === 'vitamin-b-complex')) {
          rec.relevanceScore += 0.4;
        }
        
        // Map UI values to recommendation system values for backward compatibility
        if (objective === 'immunity' && rec.categories.includes('immunité')) {
          rec.relevanceScore += 0.4;
        }
        if (objective === 'sleep' && (rec.id === 'magnesium-glycinate' || rec.id === 'circadian-rhythm-optimization')) {
          rec.relevanceScore += 0.45;
        }
        if (objective === 'concentration' && (rec.id === 'omega3-supplementation' || rec.id === 'vitamin-b-complex')) {
          rec.relevanceScore += 0.4;
        }
      });
    });
  ...
  const aiStatus = getAILearningStatusDetails();
...
export function generateRecommendations(quizData: QuizData): Recommendation[] {
  ...
  console.log("Quiz data symptoms:", quizData.symptoms);
  console.log("Quiz data objectives:", quizData.objectives);
  console.log("Quiz data lifestyle:", quizData.lifestyle);
...
// Importation des fonctions depuis aiLearning.ts
import { getAILearningStatus, getAILearningStatusDetails, evaluateDataQuality } from './aiLearning';
