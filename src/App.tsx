...
import { complianceTracker } from './utils/complianceTracker'; // Added import for complianceTracker
...
const runContentSafetyCheck = () => {
  ...
  // Auto-fix dollar amounts and commercial terms
  document.querySelectorAll('p, h1, h2, h3, h4, h5, div, button, a, span').forEach((element) => {
    if (element.textContent && /\$\d+|\d+\s?\$|€\d+|\d+\s?€|\d+\s?USD|\d+\s?EUR/g.test(element.textContent)) {
      console.warn("[GoogleAdGrantsSafety] Removing pricing information from:", element.textContent);
      element.textContent = element.textContent.replace(/\$\d+|\d+\s?\$|€\d+|\d+\s?€|\d+\s?USD|\d+\s?EUR/g, "free");
    }
  });
