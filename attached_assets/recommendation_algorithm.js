/**
 * Algorithme de recommandation nutritionnelle
...
const complementsDatabase = require('./database_complements.js');
...
class RecommendationSystem {
   ...
   * @param {Object} quizResponses - Les réponses de l'utilisateur au quiz
   * @return {Array} - Liste des compléments recommandés avec leurs scores
   */
  calculateScores(quizResponses) {
    const scores = {};
   ...
   * @private
   */
  _processSymptoms(selectedSymptoms, scores) {
   ...
   * @private
   */
  _processObjectives(selectedObjectives, scores) {
   ...
   * @private
   */
  _adjustForActivity(activity, scores) {
   ...
   * @private
   */
  _adjustForSleep(sleepQuality, scores) {
   ...
   * @private
   */
  _adjustForStress(stressLevel, scores) {
   ...
   * @private
   */
  _adjustForFruitVeg(fruitVegConsumption, scores) {
   ...
   * @private
   */
  _checkDietCompatibility(diet, scores) {
    for (const complement in complementsDatabase) {
      const complementData = complementsDatabase[complement];
   ...
   * @param {Array} sortedScores - Scores triés des compléments
   * @param {Number} limit - Nombre de recommandations à retourner
   * @return {Array} - Recommandations finales avec détails
   */
  generateRecommendations(sortedScores, limit = 5) {
   ...
   * @param {Object} quizResponses - Les réponses de l'utilisateur au quiz
   * @return {Array} - Recommandations finales
   */
  getRecommendations(quizResponses) {
